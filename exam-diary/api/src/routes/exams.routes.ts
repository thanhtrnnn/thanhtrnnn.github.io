...
import { makeId, isExamAvailableNow } from '../lib/core';
...
function sanitizeExamPayload(input: any, current: any = null): any {
  const payload = input || {};
  const base = current || {};
  const exam: any = {
    id: base.id,
...
function sanitizeQuestionPayload(input: any, fallbackExamId: string): any {
  ...
  const optionIds = new Set(normalizedOptions.map((item: any) => item.id));
...
router.get('/', async (req: any, res: any, next: any) => {
  try {
    ...
    let exams = db.exams.filter((exam: any) => {
      const byKeyword = !keyword ? true : [exam.title, exam.description].join(' ').toLowerCase().includes(keyword);
    ...
    if (req.user.role === 'student') {
      exams = exams.filter((exam: any) => exam.status === 'active');
    }

    if (availableFilter !== null) {
      exams = exams.filter((exam: any) => isExamAvailableNow(exam) === availableFilter);
    }

    const withMeta = exams
      .map((exam: any) => ({
        ...exam,
        questionCount: db.questions.filter((q: any) => q.examId === exam.id).length,
...
router.get('/:examId', async (req: any, res: any, next: any) => {
  try {
    const db = await readDb();
    const exam = db.exams.find((item: any) => item.id === req.params.examId);
...
router.get('/:examId/questions', async (req: any, res: any, next: any) => {
  try {
    const db = await readDb();
    const exam = db.exams.find((item: any) => item.id === req.params.examId);
...
router.post('/', authorize('admin'), async (req: any, res: any, next: any) => {
  try {
    ...
    if (db.exams.some((item: any) => item.id === exam.id)) {
      throw createError(409, 'Exam id already exists');
    }

    const questionsInput = Array.isArray(req.body?.questions) ? req.body.questions : [];
    const questions = questionsInput.map((item: any) => sanitizeQuestionPayload(item, exam.id));
...
router.put('/:examId', authorize('admin'), async (req: any, res: any, next: any) => {
  try {
    const db = await readDb();
    const index = db.exams.findIndex((item: any) => item.id === req.params.examId);
    ...
    if (Array.isArray(req.body?.questions)) {
      const incoming = req.body.questions.map((item: any) => sanitizeQuestionPayload(item, updatedExam.id));
      db.questions = db.questions.filter((q: any) => q.examId !== updatedExam.id);
...
router.put('/:examId/questions', authorize('admin'), async (req: any, res: any, next: any) => {
  try {
    const db = await readDb();
    const exam = db.exams.find((item: any) => item.id === req.params.examId);
    ...
    const normalized = questionsInput.map((item: any) => sanitizeQuestionPayload(item, exam.id));
    db.questions = db.questions.filter((q: any) => q.examId !== exam.id);
...
router.post('/:examId/questions/import', authorize('admin'), async (req: any, res: any, next: any) => {
  try {
    const db = await readDb();
    const exam = db.exams.find((item: any) => item.id === req.params.examId);
    if (!exam) {
...
router.delete('/:examId', authorize('admin'), async (req: any, res: any, next: any) => {
  try {
    const db = await readDb();
    const exam = db.exams.find((item: any) => item.id === req.params.examId);
    ...
    db.exams = db.exams.filter((item: any) => item.id !== exam.id);
    db.questions = db.questions.filter((q: any) => q.examId !== exam.id);
    await writeDb(db);
